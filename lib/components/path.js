...
const open = function(statwolfPath) {
  statwolfPath = statwolfPath.replace(/^statwolf:\/\//, '').replace(/[^a-zA-Z0-9\:\.\_\-]+/g, '').split(':');
...
export default function(d) {
  const copyPathHandler = function(relativePath, isExternal, line) {
    if (relativePath === null || relativePath.length === 0) {
      n.error('Statwolf path', 'Not a Statwolf module', true);
      return;
    }

    relativePath = `${ isExternal === true ? 'statwolf://' : '' }${ relativePath.replace(new RegExp('\\' + path.sep, 'g'), '.') }`;
  ...
  const onPath = atom.commands.add('atom-workspace', {
    ...
    'statwolf:copyURIAtLine': function(event) {
      const textEditor = atom.workspace.getActiveTextEditor();
      const relativePath = path.sanitize(textEditor.getPath() || '').replace(rootPath() + path.sep, '').split(path.sep).slice(0, -1).join(path.sep);
      const lineNumber = textEditor.getCursorBufferPosition().row;

      copyPathHandler(relativePath, true, lineNumber);
    },
    'statwolf:copyURI': function(event) {
      const relativePath = resolver(event.target, true);
      copyPathHandler(relativePath, true);
    },
