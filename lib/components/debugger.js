...
export default function(d) {
  ...
  dbgConsole.on('evaluate', function(code) {
    ...
    data.log.push({
      type: 'command',
      text: code,
      position: dbgConsole.getPosition(code)
    });
    ...
    if (code === 'clear()') {
      ...
      data.log.push({
        type: 'output',
        text: data.mode,
        position: dbgConsole.getPosition(data.mode)
      });
    } else {
      if (dbgr == null) {
        data.log.push({
          type: 'output',
          text: 'No debugger selected',
          position: dbgConsole.getPosition('No debugger selected')
        });
      } else {
  ...
  dbgStatus.on('reset', function() {
    ...
    const console = dbgConsole.getData();
    console.log = [];
    dbgConsole.notify();
  ...
  const connect = function(data) {
    ...
    client.on('eval', function(reply) {
      ...
      if (reply.exception) {
        data.log.push({
          type: 'error',
          text: reply.exception,
          position: dbgConsole.getPosition(reply.exception)
        });
      } else {
        if (typeof(reply.result) === 'function') {
          reply.result = reply.result.toString();
        } else if(typeof(reply.result) === 'undefined') {
        ...
        data.log.push({
          type: 'output',
          text: reply.result,
          position: dbgConsole.getPosition(reply.result)
        });
      }
